...
export type TaskBucketKey = "overdue" | "today" | "soon" | "later";
...
/**
 * 期限超過 / 今日 / そろそろ（〜soonDays日） / 期限なし・それ以降 の4分類。
 * 「期限なし」バケットには due_on が null のタスクに加え、soonDays より先の
 * タスクも含める（今日画面の役割は「今見るべきもの」に絞ること。それより先の
 * 期限はアジェンダ画面(M5)で俯瞰する）。
 */
